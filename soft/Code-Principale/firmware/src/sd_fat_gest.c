...
void sd_BNO_scheduleWrite (s_bno055_data * data)
{
    ...
    if(appFatData.state == APP_IDLE)
    {
        ...
        sprintf(appFatData.data, "%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%.4f;%d;%d;%d;%d;"
                                  ,data->gravity.x, data->gravity.y, data->gravity.z, data->gyro.x, data->gyro.y, data->gyro.z
